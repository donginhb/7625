...
extern INT16U			pow_dly_ready_flg;
...
void LED_blanking_isr(void)
{
	...
	//	return;
	static INT8U led_red_green_exc = 0, green_flick = 0;
...
void ap_peripheral_key_judge(void)
{
	...
	static INT8U  video_cnt = 0, motion_cnt = 0;
	//static INT8U  video_flag = 1, motion_flag = 1;
	
	key_down = gpio_read_io(SWITCH_KEY);
	if(pow_dly_ready_flg)
	{
		if(!key_down)
		{
			if((video_cnt++ > 10) && video_flag)
			{
				//DBG_PRINT("video status\n");
				if ((!s_usbd_pin) && (!pic_down_flag) && (!card_space_less_flag) && (!video_down_flag))
				{
					if (ap_state_handling_storage_id_get() != NO_STORAGE)
					{
						DBG_PRINT("ap_video_record_sts_get() = %d\n", ap_video_record_sts_get());
						if(ap_video_record_sts_get() & 0x4)	//移动侦测
						{
							//DBG_PRINT("video status, pre status motion\n");
							if(ap_video_record_sts_get() & 0x2)
							{
								DBG_PRINT("video status, 1\n");
								msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
							}
							else
							{
								DBG_PRINT("video status, 2\n");
		...
		{
			if((motion_cnt++ > 10)&& motion_flag)
			{
				DBG_PRINT("motion status\n");
				if (usb_state_get() != 1)
				{
					if (ap_state_handling_storage_id_get() != NO_STORAGE)
					{
						if(ap_video_record_sts_get() & 0x2)	//录像
						{
							DBG_PRINT("motion status, pre status record\n");
							msgQSend(ApQ, MSG_APQ_VIDEO_RECORD_ACTIVE, NULL, NULL, MSG_PRI_NORMAL);
						}
						msgQSend(ApQ, MSG_APQ_POWER_ON_AUTO_RECORD, NULL, NULL, MSG_PRI_NORMAL);
					}
